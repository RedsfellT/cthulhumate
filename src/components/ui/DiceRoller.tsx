...
import { Dice3DOverlay } from './Dice3D'
...
export function SkillRollButton({ skillName, value, onClose }: SkillRollProps) {
  ...
  const [overlayColor, setOverlayColor] = useState('#c8972a')
  ...
  function doRoll() {
    ...
    setTimeout(() => {
      ...
      setResult(res)
      setOverlayValue(roll)
      setOverlayLabel(res.label)
      setOverlayColor(res.color)
      setRolling(false)
